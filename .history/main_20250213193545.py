...
def draw_block(x, y, color):
    return f'<rect x="{x * BLOCK_SIZE}" y="{y * BLOCK_SIZE}" width="{BLOCK_SIZE}" height="{BLOCK_SIZE}" fill="rgb{color}" stroke="white" />'
...
def game_loop():
    ...
    while not game_over:
        ...
        if not is_valid_move(current_piece, piece_x, piece_y + 1, piece_rotation):
