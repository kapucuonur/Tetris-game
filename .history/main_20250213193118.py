...
def is_valid_move(piece, x, y, rotation):
    ...
    for dy, row in enumerate(rotated_piece):
        if not isinstance(row, list):  # Eğer bir liste değilse, atla
            continue
...
def rotate_tetromino(piece, rotation):
    try:
        ...
        return piece  # Eğer piece bir liste değilse, orijinal şeklini koru
...
# Correctly calculate piece_x
piece_x = GRID_WIDTH // 2 - len(current_piece[0][0]) // 2 if isinstance(current_piece[0][0], list) else GRID_WIDTH // 2 - 1
piece_y = 0
piece_rotation = 0
score = 0
...
def game_loop():
    ...
    while True:
        ...
        try:
            ...
            # Eğer bir hata olursa, oyun durumunu sıfırla
            current_piece, current_color = random.choice(TETROMINOS)
            piece_x = GRID_WIDTH // 2 - len(current_piece[0][0]) // 2 if isinstance(current_piece[0][0], list) else GRID_WIDTH // 2 - 1
            piece_y = 0
            piece_rotation = 0
